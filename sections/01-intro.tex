...
On the one hand, we use erasure coding as the data redundancy strategy, which stores less copies of data than other systems like HDFS, and we also use built-in tiering storage and  compression techniques to save the storage cost. Besides, for semantics of stream and batch data, we can just store one copy to further save the cost.
